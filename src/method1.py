...
def compute_tau0(A, E, A11, E11, tau_min=-20, tau_max=2, tau_number=300):
    ...
    for tau in taus:
        ...
        rhs = norm_A11 / (tau + 0.5 * sigma_min_E11)
...
def method_1_bounds_th_1_1(A, E, A12, A21, E11, delta=1e-15,
    ...
    for j, tau in enumerate(taus):
        ...
        lower = np.sqrt(sigma_min_A12A21 / ((1.5 * norm_E11 + 2 * tau) * tau)) - gamma
        upper = np.sqrt(norm_A12A21 / (0.5 * sigma_min_E11 * tau)) + gamma
...
def method_1_bounds_cor_2_3(A, E, delta=1e-15,
    ...
    for j, tau in enumerate(taus):
        ...
        lower = sigma_min_A / (kappa_V_E * np.sqrt(1.5 * norm_E * kappa_V_E + 2 * tau) * np.sqrt(tau)) - gamma
        upper = norm_A * kappa_V_E32 / np.sqrt(0.5 * sigma_min_E * tau) + gamma
...
def method_1_bounds_cor_3_3(E, J, R, Q, delta=1e-15,
                            tau_min=-20, tau_max=2, tau_number=300):
    """Bounds from Corollary 3.3 (port-Hamiltonian structure)."""
    ...
    Q_half = sqrtm(Q)
    Q_half_inv = np.linalg.inv(Q_half)
    Q_inv = np.linalg.inv(Q)

    norm_J = np.linalg.norm(Q_half @ J @ Q_half_inv)
    norm_E = np.linalg.norm(Q_half @ E @ Q_half_inv)
    norm_A = np.linalg.norm(Q_half @ A @ Q_half_inv)

    sigma_min_J = svdvals(Q_inv.T @ J.T @ Q.T @ J)[-1]
    sigma_min_E = svdvals(Q_inv.T @ E.T @ Q.T @ E)[-1]
    ...
    for j, tau in enumerate(taus):
        _, V = eig(E + tau * I, A)
        kappa_V = cond(Q_half @ V @ Q_half_inv)

        gamma = delta * kappa_V * (tau + norm_A) / tau / (tau - delta)

        lower = sigma_min_J / (np.sqrt(1.5 * norm_E + 2 * tau) * np.sqrt(tau)) - gamma
        upper = norm_J / (np.sqrt(0.5 * sigma_min_E) * np.sqrt(tau)) + gamma
